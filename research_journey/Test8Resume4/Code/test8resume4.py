...
BASE_PATH = "/home/burak/cifar100-effnetv2-90.20acc-mobile-inference/research_journey/Test8Resume4"
...
CHECKPOINT_SAVE_PATH = os.path.join(WEIGHTS_DIR, "user_best_model_12.pth")

# 3. LAST_CHECKPOINT_PATH: Rolling checkpoint updated every epoch for crash recovery
LAST_CHECKPOINT_PATH = os.path.join(WEIGHTS_DIR, "user_last_checkpoint_12.pth")

# 4. FINAL_MODEL_SAVE_PATH: Saves only the model state_dict after the final epoch
FINAL_MODEL_SAVE_PATH = os.path.join(WEIGHTS_DIR, "user_last_epochs_12.pth")

# 5. PLOT_SAVE_PATH: File path to save the training/validation curves as an image
PLOT_SAVE_PATH = os.path.join(PLOT_DIR, "user_training_history_plot_12.png")
# --------------------------
...
class FocalLoss(nn.Module):
    ...
    hard, misclassified ones by multiplying the standard cross-entropy loss
    by a modulating factor (1 - pt)^gamma, where pt is the model's estimated
    probability for the correct class.

    Args:
        gamma (float): Focusing parameter. Higher values increase the penalty
                       on easy examples. gamma=0 reduces to standard CE loss.
        label_smoothing (float): Label smoothing factor applied inside CE loss.
        alpha (Tensor or None): Optional per-class weight tensor of shape
                                (num_classes,). When provided, each sample's
                                loss is multiplied by the weight of its true
                                class, allowing harder classes to be penalised
                                more heavily.
    """
    ...
    def __init__(self, gamma=2.0, label_smoothing=0.1, alpha=None):
        ...
        # alpha: optional per-class weighting tensor (useful for hard classes)
        self.alpha = alpha

    def forward(self, inputs, targets):
        # Compute per-sample cross-entropy with label smoothing (no reduction yet)
        ...
        # Compute pt: the model's confidence on the correct class
        pt = torch.exp(-ce_loss)

        # Apply focal modulating factor: down-weight easy examples
        focal_loss = ((1 - pt) ** self.gamma) * ce_loss

        # If per-class alpha weights are provided, scale each sample's loss
        # by the weight assigned to its true class index
        if self.alpha is not None:
            at = self.alpha[targets]  # Select alpha for each sample's true class
            focal_loss = at * focal_loss

        return focal_loss.mean()  # Return scalar mean loss over the batch
...
def main():
    ...
    # Same light pipeline as v11: RandAugment magnitude=4, RandomErasing disabled
    transform_train = transforms.Compose([
        ...
        #transforms.RandomErasing(p=0.2, scale=(0.01, 0.05), ratio=(0.2, 3))  # Disabled for fine-tuning
    ])
    ...
    # Fully disabled (set to None) — hard labels are used so Focal Loss can
    # correctly compute per-sample pt values from ground-truth class indices
    mixup_fn = None

    # --- Per-Class Alpha Weights for Focal Loss ---
    # Retrieve the ordered class name list from the training dataset
    classes = train_dataset.classes

    # Start with uniform weight 1.0 for all 100 classes
    alpha_weights = torch.ones(100).to(device)

    # Double the loss penalty for the 10 historically hardest classes identified
    # from previous per-class accuracy analysis runs
    hard_classes = ['girl', 'boy', 'woman', 'man', 'oak_tree', 'willow_tree',
                    'possum', 'maple_tree', 'pine_tree', 'seal']
    for cls_name in hard_classes:
        if cls_name in classes:
            idx = classes.index(cls_name)
            alpha_weights[idx] = 2.0  # 2x penalty for errors on these classes

    # --- Loss Function and Optimizer ---
    # FocalLoss replaces standard CrossEntropyLoss in this stage.
    # gamma=2.0 focuses training on hard examples; alpha applies class weighting.
    # LR (1e-6) and weight_decay (1e-2) are both more aggressive than v11
    # to apply tighter regularisation on the hardest-class corrections.
    criterion = FocalLoss(gamma=2.0, label_smoothing=0.1, alpha=alpha_weights)
    num_epochs = 10   # Short targeted run — just enough to correct hard classes
    base_lr = 1e-6    # Extremely low LR; even smaller than v11 (3e-6)

    # Standard AdamW — weight_decay increased to 1e-2 vs 1e-4 in previous versions
    ...
    # This script is a targeted fine-tuning stage focused on hard classes.
    # It loads the best checkpoint from the previous run (PRETRAINED_WEIGHT_PATH)
    # and resets all training state (history, optimizer, epoch counter) to start
    # a clean short run. Unlike a resume, no optimizer or scheduler state is restored.
    start_epoch = 0

    if os.path.exists(PRETRAINED_WEIGHT_PATH):
        print(f"\n--- Loading pretrained weights for hard-class fine-tuning... ---")
    ...
    log_dir = os.path.join(BASE_PATH, f"user_runs12/baseline_{current_time}")
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for images, labels in train_loader:
            ...
            # mixup_fn is None in this stage — hard labels required by FocalLoss
            ...
            # ndim == 2 check is kept for safety even though mixup_fn is None
            if labels.ndim == 2:
