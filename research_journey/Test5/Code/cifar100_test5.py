...
BASE_PATH = "/home/burak/cifar100-effnetv2-90.20acc-mobile-inference/research_journey/Test5"
...
CHECKPOINT_SAVE_PATH = os.path.join(WEIGHTS_DIR, "user_best_model_5.pth")

# 2. FINAL_MODEL_SAVE_PATH: Saves only the model state_dict after the final epoch.
FINAL_MODEL_SAVE_PATH = os.path.join(WEIGHTS_DIR, "user_last_epochs_5.pth")

# 3. PLOT_SAVE_PATH: The file path where the training and validation Accuracy/Loss curves will be saved.
PLOT_SAVE_PATH = os.path.join(PLOT_DIR, "user_training_history_plot_5.png")
# --------------------------
...
def main():
    ...
    # --- Model Setup ---
    # Load EfficientNetV2-S pretrained on ImageNet, replace final classifier for 100 classes
    ...
    mixup_fn = Mixup(
        ...
        label_smoothing=0.1,  # Smooth one-hot labels to reduce overconfidence
        num_classes=100
    )

    # --- Loss Function and Optimizer ---
    # CrossEntropyLoss with label smoothing to complement Mixup's soft labels
    ...
    log_dir = os.path.join(BASE_PATH, f"user_runs5/baseline_{current_time}")
