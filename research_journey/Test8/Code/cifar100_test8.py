...
class SAM(torch.optim.Optimizer):
    ...
    def __init__(self, params, base_optimizer, rho=0.035, adaptive=False, **kwargs):
...
def main():
    ...
    # RandomResizedCrop → RandAugment → Random flip → Normalize → Random erasing
    ...
    # Applied with reduced strength (prob=0.7) compared to previous versions;
    # label_smoothing is intentionally omitted here (handled by the loss criterion)
    mixup_fn = Mixup(
        mixup_alpha=0.4,      # Mixup interpolation strength
        cutmix_alpha=0.7,     # CutMix interpolation strength
        prob=0.7,             # Probability of applying an augmentation each batch
    ...
    # SAM wraps AdamW as its base optimizer; rho=0.035 is a slightly tighter
    # perturbation radius compared to the previous version (rho=0.05)
    criterion = nn.CrossEntropyLoss(label_smoothing=0.1)
    base_optimizer = optim.AdamW
    optimizer = SAM(model.parameters(), base_optimizer, rho=0.035, lr=6e-4, weight_decay=1e-4)
    num_epochs = 200
    ...
    scheduler = torch.optim.lr_scheduler.OneCycleLR(
        ...
        final_div_factor=1e3,   # Min LR = initial_lr / final_div_factor
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for images, labels in train_loader:
            ...
            # --- SAM STEP 1: Forward pass, compute loss, and climb toward loss peak ---
            outputs = model(images)
            loss = criterion(outputs, labels)

            # Compute gradients for the perturbation step
            loss.backward()

            # Perturb weights toward the sharpest direction within the rho-ball
            optimizer.first_step(zero_grad=True)

            # --- SAM STEP 2: Second forward+backward at the perturbed point,
            #     then restore original weights and apply the real flat-valley update ---

            # Re-run forward pass at the perturbed weights (this is SAM's core mechanism)
            new_outputs = model(images)
            new_loss = criterion(new_outputs, labels)

            # Compute gradients at the perturbed point
            new_loss.backward()

            # Clip gradients before the second step to prevent large updates
            torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)

            # Restore original weights and apply the base optimizer update.
            # NOTE: second_step internally calls base_optimizer.step(), so no
            # separate optimizer.step() call is needed here.
        ...
        # Key names use 'model_state_dict' / 'optimizer_state_dict' to stay
        # consistent with the rolling checkpoint format used for resumption.
