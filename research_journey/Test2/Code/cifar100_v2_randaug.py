...
BASE_PATH = "/home/burak/cifar100-effnetv2-90.20acc-mobile-inference/research_journey/Test2"
...
CHECKPOINT_SAVE_PATH = os.path.join(WEIGHTS_DIR, "user_best_model_2.pth")

# 2. FINAL_MODEL_SAVE_PATH: Saves only the model state_dict after the final epoch.
FINAL_MODEL_SAVE_PATH = os.path.join(WEIGHTS_DIR, "user_last_epochs_2.pth")

# 3. PLOT_SAVE_PATH: The file path where the training and validation Accuracy/Loss curves will be saved.
PLOT_SAVE_PATH = os.path.join(PLOT_DIR, "user_training_history_plot_2.png")
# --------------------------
...
def seed_everything(seed=42):
    ...
    Also sets cuDNN to deterministic mode to avoid non-deterministic operations.
    """
    random.seed(seed)
...
def find_optimal_batch_size(model, input_shape=(3, 224, 224), device="cuda", threshold=0.8):
    ...
    usage exceeds the specified threshold or an OOM error is encountered.
    ...
    """
    model.to(device)
    model.train()  # Train mode is used because activations are stored during forward pass
    ...
    try:
        while True:
            # Clear cached memory and reset peak memory statistics before each test
            torch.cuda.empty_cache()
            torch.cuda.reset_peak_memory_stats()

            # Generate random dummy inputs and integer class labels
            ...
            # Check peak memory allocated during this pass
            ...
            # Safety cap to avoid testing unreasonably large batch sizes
            if batch_size > 2048:
                break
    ...
    # Release all cached memory and clear gradients before returning
    torch.cuda.empty_cache()
    model.zero_grad()
...
def analyze_per_class_accuracy(model, loader, device, classes):
    """
    Evaluates the model's per-class accuracy on the given data loader and
    prints the 10 classes with the lowest accuracy (i.e., the hardest to classify).

    Args:
        model:   The trained PyTorch model to evaluate.
        loader:  DataLoader for the evaluation split (typically validation set).
        device:  Device to run inference on.
        classes: List of class name strings, indexed by class label integer.
    """
    model.eval()
    ...
    # Compute accuracy for each class and sort by ascending accuracy
    accs = []
    for i in range(100):
...
def main():
    ...
    # Training transform: resize, RandAugment, normalize, and random erasing for regularization
    ...
    # Batch size can be found dynamically via find_optimal_batch_size(), but is fixed here for stability
    ...
    # OneCycleLR: linearly warms up to max_lr then cosine-anneals to near zero
    # pct_start=0.3 means 30% of training is warmup; div_factor sets initial lr = max_lr / 25
    ...
    log_dir = os.path.join(BASE_PATH, f"user_runs2/baseline_{current_time}")
    ...
    for epoch in range(num_epochs):
        ...
        for images, labels in train_loader:
            ...
            if labels.ndim == 2:  # Soft labels: compare argmax of both
        ...
        # Save the model checkpoint whenever a new best validation loss is achieved
