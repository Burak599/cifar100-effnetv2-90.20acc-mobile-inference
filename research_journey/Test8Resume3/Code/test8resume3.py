...
BASE_PATH = "/home/burak/cifar100-effnetv2-90.20acc-mobile-inference/research_journey/Test8Resume3"
...
PRETRAINED_WEIGHT_PATH = os.path.abspath(os.path.join(BASE_PATH , "..", "Test8Resume2", "user_weights", "user_best_model_10.pth"))

os.makedirs(WEIGHTS_DIR, exist_ok=True)
os.makedirs(PLOT_DIR, exist_ok=True)

# 2. CHECKPOINT_SAVE_PATH: Saves the full training state whenever validation loss improves
CHECKPOINT_SAVE_PATH = os.path.join(WEIGHTS_DIR, "user_best_model_11.pth")

# 3. LAST_CHECKPOINT_PATH: Rolling checkpoint updated every epoch for crash recovery
LAST_CHECKPOINT_PATH = os.path.join(WEIGHTS_DIR, "user_last_checkpoint_11.pth")

# 4. FINAL_MODEL_SAVE_PATH: Saves only the model state_dict after the final epoch
FINAL_MODEL_SAVE_PATH = os.path.join(WEIGHTS_DIR, "user_last_epochs_11.pth")

# 5. PLOT_SAVE_PATH: File path to save the training/validation curves as an image
PLOT_SAVE_PATH = os.path.join(PLOT_DIR, "user_training_history_plot_11.png")
# --------------------------
...
def main():
    ...
    # Lightest augmentation in the series: RandAugment magnitude reduced from 7 to 4,
    # and RandomErasing is fully disabled (commented out) to preserve fine-grained
    # patterns in a nearly converged model
    transform_train = transforms.Compose([
        ...
        #transforms.RandomErasing(p=0.2, scale=(0.01, 0.05), ratio=(0.2, 3))  # Disabled for ultra fine-tuning
    ])
    ...
    # Fully disabled (set to None) for this ultra fine-tuning stage.
    # Hard labels are used to allow the model to make precise, clean corrections
    # without any label interpolation noise.
    mixup_fn = None

    # --- Loss Function and Optimizer ---
    # Ultra fine-tuning stage: even smaller LR (3e-6) than v9/v10 (8e-6),
    # and only 10 epochs — just enough for a final accuracy squeeze
    criterion = nn.CrossEntropyLoss(label_smoothing=0.1)
    num_epochs = 10   # Short run — ultra fine-tuning needs very few epochs
    base_lr = 3e-6    # Extremely low LR for surgical weight adjustments
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=num_epochs, eta_min=5e-7)
    ...
    # This script is an ultra fine-tuning stage: it loads the best checkpoint
    # from the previous run (PRETRAINED_WEIGHT_PATH) and resets all training
    # state (history, optimizer, epoch counter) to start a clean short run.
    # Unlike a resume, no optimizer or scheduler state is restored.
    start_epoch = 0

    if os.path.exists(PRETRAINED_WEIGHT_PATH):
        print(f"\n--- Loading pretrained weights for ultra fine-tuning... ---")
    ...
    log_dir = os.path.join(BASE_PATH, f"user_runs11/baseline_{current_time}")
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for images, labels in train_loader:
            ...
            # mixup_fn is None in this stage — hard labels are used directly
